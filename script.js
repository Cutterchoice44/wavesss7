...
function initWaveform() {
  ...
  audioCtx.resume().then(() => {
    ...
    function draw() {
      ...
      ctx.lineWidth = 3;
      ctx.strokeStyle = 'rgba(90,135,133,0.6)';
      ctx.beginPath();
...
window.addEventListener('click', initWaveform, { once: true });
// ───────────────────────────────────────────────────────────────────────────── */

// ─── REAL-TIME WAVEFORM VISUALIZATION ON PLAY ───────────────────────────────
let audioCtx, analyser, dataArray, bufferLength, audioEl;
function initWaveform() {
  if (audioCtx) return;
  audioCtx = new (window.AudioContext || window.webkitAudioContext)();
  audioCtx.resume().then(() => {
    audioEl = new Audio('https://cutters-choice-radio.radiocult.fm/stream');
    audioEl.crossOrigin = 'anonymous';
    audioEl.muted = true;  // Mute waveform audio
    audioEl.play().catch(e => console.error('Playback error:', e));
    const source = audioCtx.createMediaElementSource(audioEl);
    analyser = audioCtx.createAnalyser();
    analyser.fftSize = 256;
...
// Trigger waveform init when user clicks the live-player container (i.e., when play button is used)
const playerContainer = document.querySelector('.live-player .player-container');
if (playerContainer) {
  playerContainer.addEventListener('click', initWaveform, { once: true });
}
// ───────────────────────────────────────────────────────────────────────────── */
